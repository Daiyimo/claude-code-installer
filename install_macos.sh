...
DOWNLOAD_URL="https://claude.ai/install/macos/claude"
TEMP_FILE="/tmp/claude-install"

if curl -fsSL "$DOWNLOAD_URL" -o "$TEMP_FILE" 2>/dev/null || curl -fsSL "$DOWNLOAD_URL" -o "$TEMP_FILE"; then
    chmod +x "$TEMP_FILE"
...
if ! echo "$PATH" | grep -q "$INSTALL_DIR"; then
    ...
    SHELL_RC="$HOME/.zshrc"
    if [ ! -f "$SHELL_RC" ]; then SHELL_RC="$HOME/.bash_profile"; fi

    echo "" >> "$SHELL_RC"
    echo 'export PATH="$HOME/.local/bin:$PATH"' >> "$SHELL_RC"
    export PATH="$INSTALL_DIR:$PATH"
    success "已将 $INSTALL_DIR 添加到 PATH ($SHELL_RC)"
fi
